...
describe('Ahorcado - marcador de la sesión', () => {
  it('acumula victorias y derrotas entre partidas consecutivas', () => {
    ...
    juego.adivinar('O');
    juego.adivinar('S');
    expect(juego.resultadoSesion()).toEqual({ victorias: 1, derrotas: 0 });

    juego.reiniciar();

    ['A', 'B', 'C', 'D', 'E', 'F'].forEach((letra) => juego.adivinar(letra));
    expect(juego.resultadoSesion()).toEqual({ victorias: 1, derrotas: 1 });
  });
});
